...
class TestAgentsCycle(unittest.TestCase):
    ...
    def test_full_debugging_cycle_success(self, mock_observer_class, mock_analyst_class, mock_verifier_class, mock_code_graph_builder_build):
        """
        ...
        dummy_file = CodeGraphFile(path="math_ops.py", hash="", loc=0)
        dummy_code_graph = CodeGraph(language="Python", manifest=[dummy_file])
        mock_code_graph_builder_build.return_value = dummy_code_graph
        ...
        # Set provider and model name for performance record
